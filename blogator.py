...
def read_blog_meta(blog_file_path):
    ...
    with blog_file_path.open() as fin:
        ...
        return {
            ...
            'favicon'      : 'favicon.ico'  if favicon_file is not None 
                                            else md_meta_get(meta, 'favicon-url', 
...
def generate(blog, args):
    ...
    for post in posts:
        write_templated(args.templates / "post.template.html", args.target / post['link_base'], 
            {'blog': blog, 'posts': posts, 'post': post})    

    write_templated(args.templates / "index.template.html", args.target / "index.html", 
